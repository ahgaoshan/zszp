...
export function MemberDetailDialog({
  ...
  // 获取日期显示（优先农历/传统纪年，其次公历）
  const getDateDisplay = (calendar: string | null, gregorian: string | null) => {
    if (calendar) return calendar;
    const gregorianFormatted = formatDate(gregorian);
  ...
  const getNameDetail = () => {
    ...
    if (member.pseudonym) {
      parts.push(`号${member.pseudonym}`);
    }
    return parts.length > 0 ? parts.join("，") : null;
  };

  const nameDetail = getNameDetail();
                ...
                <div className="bg-stone-100/50 dark:bg-stone-800/50 p-4 sm:p-6 border-b border-stone-200 dark:border-stone-700 flex items-center justify-between shrink-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {nameDetail && (
                        <p className="text-xs sm:text-sm text-stone-500 dark:text-stone-400 font-serif mt-0.5">
                          {nameDetail}
                        </p>
                      )}
                    ...
                    <div className="grid grid-cols-2 gap-4 sm:gap-6">
                      <div className="space-y-1">
                        ...
                        <p className="text-base sm:text-lg text-stone-700 dark:text-stone-300">
                          {getDateDisplay(member.birthday_calendar, member.birthday)}
                        </p>
                      </div>
                      {!member.is_alive && (
                        <div className="space-y-1">
                          ...
                          <p className="text-base sm:text-lg text-stone-700 dark:text-stone-300">
                            {getDateDisplay(member.death_date_calendar, member.death_date)}
                          </p>
                        </div>
                      )}
                    </div>
