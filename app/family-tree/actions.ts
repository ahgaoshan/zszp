...
export async function createFamilyMember(
  ...
  const { error } = await supabase.from("family_members").insert({
    ...
    birthday_calendar: input.birthday_calendar || null,
    death_date_calendar: input.death_date_calendar || null,
...
export async function batchCreateFamilyMembers(
  ...
  const insertPayload = members.map((m) => {
    ...
    return {
      ...
      remarks: m.remarks,
      birthday: m.birthday,
      birthday_calendar: m.birthday_calendar || null,
      death_date_calendar: m.death_date_calendar || null,
...
export async function fetchMembersForTimeline(): Promise<
  { id: number; name: string; birthday: string | null; death_date: string | null; birthday_calendar: string | null; death_date_calendar: string | null; generation: number | null }[]
> {
  const supabase = await createClient();

  const { data, error } = await supabase
    .from("family_members")
    .select("id, name, birthday, death_date, birthday_calendar, death_date_calendar, generation")
